...
onMounted(() => {
    ...
    new QWebChannel(qt.webChannelTransport, function (channel) {
        proxyService = channel.objects.proxyService;// Retrieve the object registered in Qt
        getBookList(1)
    })
})
...
function getBookList(page) {
    ...
    proxyService.executeService('BookService', 'getBookList', {
        ...
        if (data.recordsFiltered % data.pageCount === 0) {// count the page number
